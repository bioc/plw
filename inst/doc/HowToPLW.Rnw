...
For example, the \verb*|rma| function performs all three steps in one call and returns an \verb*|exprSet| object holding
...
<<echo=TRUE,results=hide>>=
tempFile<-tempfile("ApoAI")
download.file(url="http://www.math.chalmers.se/~astrandm/plw/ApoAI.RData",
              destfile=tempFile)
load(tempFile)
require(limma)
